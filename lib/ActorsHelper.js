...
const filmsInCommonFilename = 'films-in-common.json';
const filmsInCommonFilePath = path.resolve(__dirname, '../static/json/' + filmsInCommonFilename);
...
export const writeFilmsInCommonToFile = (filmsInCommonToWrite) => {
  return new Promise((res, rej) => {
    fs.writeFile(filmsInCommonFilePath, JSON.stringify(filmsInCommonToWrite, null, 2), 'utf8', (err) => {
      if (err) {
        console.log(err.message);
        rej(err);
        return;
      ...
      filmsInCommon = filmsInCommonToWrite;
      res();
    });
  });
};
